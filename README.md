...
Either `heartbeat_url` or `heartbeat_port` (or both) can be configured. Heartbeats are sent every minute, and the system is considered unhealthy if no `Alive()` call is made within 2 minutes.
